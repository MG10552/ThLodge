...
public class ActionBarInput : MonoBehaviour {
    ...
    void Start() {
        if (mesh == null) {
            ...
            GetComponent<Renderer>().material = new Material(Resources.Load("ActionBarShaderPickup") as Shader);
            GetComponent<Renderer>().material.SetTexture("_Button", ActionBarSettings.Instance.ButtonTexture);
    ...
    void SetIcon(int atlasIndex, int iconIndex, int size) {
        ...
        GetComponent<Renderer>().material.SetTexture("_Atlas", ActionBarSettings.Instance.GetAtlasMaterial(atlasIndex).GetTexture("_Atlas"));
